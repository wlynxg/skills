...
- 核心优先的输出约束：普通问答和状态确认不套交付模板，证据按需展开，最终答复以两行简短自检暴露不确定和遗漏。
...
每个切片必须有可独立验证的结果。普通切片可连续执行；涉及安全、数据、公共 API、迁移、并发或不可逆操作时暂停等待确认。聊天中的交付摘要先给结论，只按需列出关键变更、实际验证和会影响决策的未验证风险；完整 diff、文件清单和证据保留在 review packet 中供需要时查看。
...
- 对“是否已移除”之类的状态确认，先直接回答结论，再追加两行简短且真实的不确定/遗漏自检；严格格式请求除外。
...
- 默认测试策略：风险驱动；TDD 按需。
- 默认输出策略：先回答核心问题；普通问答、确认和状态查询保持简短，实际改动只报告必要的变更、验证和风险；最终答复额外用两行真实自检暴露不确定和遗漏，细节按需展开。
